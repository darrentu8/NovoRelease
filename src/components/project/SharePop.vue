<template>
  <q-btn @click="showdialogShare = true" style="width:50px" class="full-height" dense flat icon="share"
    color="brand-grey" />

  <q-dialog v-model="showdialogShare">
...
export default {
  ...
  methods: {
    ...
    getShareList(PID) {
      this.loading = true
      this.$store.dispatch('project/getProjectShareList', PID)
        .then(() => {
          this.loading = false
        })
    },
