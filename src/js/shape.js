import { adjustCursor, canvas, getCanvasFrontIndex, canvasBrush } from './canvas'
...
const drawCircle = (moveFromDraw, moveToDraw) => {
    const radius = pointToPointDistance(moveFromDraw, moveToDraw) / Math.sqrt(2) / 2
    const circle = new fabric.Circle({
        left: moveFromDraw.x < moveToDraw.x ? moveFromDraw.x : moveToDraw.x,
        top: moveFromDraw.y < moveToDraw.y ? moveFromDraw.y : moveToDraw.y,
        stroke: canvasBrush.color,
        strokeWidth: canvasBrush.size,
        fill: 'transparent',
        evented: false,
