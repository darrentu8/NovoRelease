  ...
  <q-dialog :model-value="isShow" @before-hide="hideDialog" @before-show="initData">
    <q-card style="width: 100%;">
      ...
      <q-card-section class="q-mx-lg q-my-md">
        <q-form ref="Form" class="q-gutter-md" @submit.stop="editNt">
...
const initData = () => {
  data.id = ntStore.currentNt.id
  data.version = ntStore.currentNt.version
  data.publishDate = format(new Date(ntStore.currentNt.publishDate), 'yyyy/MM/dd')
  data.releaseNote = ntStore.currentNt.releaseNote
  data.model = ntStore.currentNt.model
  data.applist = ntStore.currentNt.applist.split(',').map(Number)
}
...
const editNt = () => {
  Form.value.validate().then(success => {
    if (success) {
      ...
      ntStore.editNt(data.id, Data).then(() => {
        hideDialog()
      })
    }
  })
